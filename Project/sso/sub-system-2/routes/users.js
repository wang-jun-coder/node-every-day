...
router.get('/login', function (ctx, next) {
  ...
  const callback = 'http://localhost:3002/users/checkToken';
...
router.get('/logout', function (ctx, next) {
    ...
    let logoutUrl = `http://localhost:3000/users/logout?redirectUrl=${encodeURIComponent('http://localhost:3002/')}&token=${ctx.session.token}`;
